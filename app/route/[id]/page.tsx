    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      <header className="panel-surface rounded-[28px] p-6">
        ...
        <div className="mt-3 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-[-0.03em] text-[color:var(--text-primary)]">{facility.name}</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[color:var(--text-secondary)]">
              선택 기관 기준으로 교통약자 차량 현황과 전국 초정밀 버스 위치 정보를 병렬로 보여줍니다.
            </p>
          </div>
          <Link href="/facilities" className="text-sm font-medium text-[color:var(--text-primary)] underline">
            목록으로
          </Link>
        </div>
      </header>
      ...
      <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="panel-surface rounded-[28px] p-6">
          <p className="text-sm font-semibold text-[color:var(--text-secondary)]">기관 정보</p>
