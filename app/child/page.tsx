  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-screen-sm flex-col">
      ...
      <header className="flex items-center gap-3 px-5 py-4">
        <Link href="/" className="rounded-full p-2 text-[color:var(--text-secondary)] hover:bg-black/5">
          ←
        </Link>
        <div>
          <p className="text-xs text-[color:var(--text-secondary)]">아이 모드</p>
          <h1 className="text-base font-bold text-[color:var(--text-primary)]">혼자가도 괜찮아 🧒</h1>
        </div>
        {step === "navigate" && sessionCode && (
          <div className="ml-auto panel-line rounded-[20px] px-3 py-1.5 text-center">
            <p className="text-xs text-[color:var(--text-secondary)]">부모님 코드</p>
            <p className="text-lg font-bold tracking-widest text-[color:var(--text-primary)]">{sessionCode}</p>
          </div>
        )}
      </header>
      ...
      <div className="flex-1 px-5 pb-8">
        ...
        {step === "locate" && (
          <div className="flex flex-col items-center gap-6 pt-12 text-center">
            <div className="flex h-20 w-20 items-center justify-center rounded-full bg-[color:var(--accent-primary)]/12">
              <span className="text-4xl">📍</span>
            </div>
            <div>
              <h2 className="text-2xl font-bold text-[color:var(--text-primary)]">지금 어디 있어?</h2>
              <p className="mt-2 text-sm text-[color:var(--text-secondary)]">현재 위치를 확인할게!</p>
            </div>
            {geoError && (
              <div className="w-full rounded-[20px] bg-rose-50 border border-rose-200 p-4 text-xs text-rose-700">
                {geoError}
              </div>
            )}
            <button
              type="button"
              onClick={locate}
              className="w-full rounded-[24px] bg-[color:var(--accent-primary)] py-5 text-lg font-bold text-white shadow-lg transition active:scale-95"
            >
        ...
        {step === "select" && (
          <div className="flex flex-col gap-6 pt-6">
            <div className="text-center">
              <h2 className="text-2xl font-bold text-[color:var(--text-primary)]">어디 갈 거야?</h2>
              <p className="mt-1 text-sm text-[color:var(--text-secondary)]">목적지를 선택해줘!</p>
            </div>
            <div className="flex flex-col gap-4">
              {DESTINATIONS.map((d) => (
                <button
                  ...
                  className="flex items-center gap-4 rounded-[24px] border border-black/8 bg-white/80 p-5 text-left shadow-sm transition hover:border-[color:var(--accent-primary)]/40 hover:bg-white active:scale-95"
                >
                  <span className="flex h-12 w-12 items-center justify-center rounded-[20px] bg-[color:var(--accent-primary)]/10 text-2xl">{d.icon}</span>
                  <div>
                    <p className="text-xl font-bold text-[color:var(--text-primary)]">{d.label}</p>
                    <p className="text-xs text-[color:var(--text-secondary)]">여기로 갈게요</p>
                  </div>
                  <span className="ml-auto text-2xl text-[color:var(--text-secondary)]">→</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {step === "navigate" && dest && (
          <div className="flex flex-col gap-4 pt-4">
            <div className="rounded-[24px] bg-[color:var(--accent-primary)]/10 border border-[color:var(--accent-primary)]/20 p-4 text-center">
              <p className="text-sm font-semibold text-[color:var(--accent-primary)]">
                {dest.icon} {dest.label}(으)로 이동 중!
              </p>
              <p className="mt-1 text-xs text-[color:var(--accent-primary)]/70">
                부모님이 내 위치를 보고 있어요 👀
              </p>
            </div>
            ...
            <div
              ref={mapRef}
              className="h-56 w-full overflow-hidden rounded-[24px] border border-black/8 bg-black/5"
              style={{ minHeight: 224 }}
            >
              {mapState === "idle" && (
                <div className="flex h-full items-center justify-center text-sm text-[color:var(--text-secondary)]">
                  <div className="flex flex-col items-center gap-2">
                    <div className="h-5 w-5 animate-spin rounded-full border-2 border-black/10 border-t-[color:var(--accent-primary)]" />
                    지도 로딩 중...
                  </div>
                </div>
              )}
            ...
            <div className="panel-surface rounded-[24px] p-5">
              <p className="text-sm font-semibold text-[color:var(--text-primary)]">📢 안내</p>
              <ul className="mt-2 flex flex-col gap-2 text-sm text-[color:var(--text-secondary)]">
            ...
            {/* 도착 알림 */}
            {arrived && (
              <div className="rounded-[24px] bg-[color:var(--accent-primary)] p-5 text-center shadow-lg">
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="w-full rounded-[24px] border border-[color:var(--accent-primary)]/30 bg-white/80 py-4 text-sm font-semibold text-[color:var(--accent-primary)] shadow-sm transition active:scale-95 hover:border-[color:var(--accent-primary)]/60"
              >
                🔊 AI 음성 안내 받기
              </button>
              {voiceText && (
                <div className="rounded-[20px] bg-[color:var(--accent-primary)]/10 border border-[color:var(--accent-primary)]/20 px-4 py-3 text-sm text-[color:var(--accent-primary)]">
                  💬 {voiceText}
                </div>
              )}
            </div>
            ...
            {pos && (
              <div className="panel-line rounded-[20px] p-3 text-center text-xs text-[color:var(--text-secondary)]">
